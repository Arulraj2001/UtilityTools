...
export const normalizeProviderName = (providerName) => String(providerName || '').trim().toLowerCase()

export const hasStoredKey = (provider = {}) => Boolean(provider.has_api_key || provider.hasApiKey)

export const getProviderName = (provider = {}) => normalizeProviderName(provider.provider_name)

export const safeProviderPayload = (provider = {}) => ({
  id: provider.id || null,
...
export const getAvailableModelIds = (provider) => (
...
export const chooseProviderModel = (provider = {}) => {
  ...
  const shouldBypassConfigured = /rate.?limit|temporarily rate-limited|insufficient balance|not found|invalid model/.test(lastError)
...
export const providerHealthPenalty = (provider = {}) => {
