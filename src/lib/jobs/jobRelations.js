...
export const matchRelatedTools = async (job, { limit = 6 } = {}) => {
  ...
  if (allResults.length < 3 && job.category) {
    try {
      const catTools = await getTools({ published: true, limit: 24 })
      catTools.forEach(t => {
        if (t.category_id || (t.category && t.category.toLowerCase() === (job.category || '').toLowerCase())) {
          allResults.push(t)
        }
      })
    } catch (e) {}
  ...
  // Rank by relevance
  const ranked = rankByRelevance(job, unique).slice(0, limit)
