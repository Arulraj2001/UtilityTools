...
import { Box, Leaf, TrendingUp, TrendingDown, DollarSign, Download } from 'lucide-react'
...
import { MetricCard, AnimatedGauge, CostDonut, TrendLine, CardSkeleton } from '../PremiumCharts'
import { PACKAGING_TYPES } from '@/lib/logistics/premiumLogisticsMath'

const PackagingProfitAnalyzer = memo(() => {
  const [inputs, setInputs] = useState({ quantity: 100 })
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
          { name: 'boxCost', label: 'Box (₹)', default: 15 },
          { name: 'tapeCost', label: 'Tape (₹)', default: 2 },
          { name: 'fillerCost', label: 'Filler (₹)', default: 3 },
          { name: 'labelCost', label: 'Label (₹)', default: 1 },
          { name: 'laborCost', label: 'Labor (₹)', default: 5 },
        ].map(f => (
          <div key={f.name} className="space-y-1">
            <label className="text-[10px] text-muted-foreground">{f.label}</label>
            <input type="number" value={inputs[f.name] ?? f.default} onChange={e => handleChange(f.name, e.target.value)}
              className="w-full px-2 py-1.5 rounded-lg bg-muted/50 border border-border/50 text-xs outline-none" />
          </div>
        ))}
        <div className="space-y-1">
          <label className="text-[10px] text-muted-foreground">Qty</label>
          <input type="number" value={inputs.quantity || 100} onChange={e => handleChange('quantity', e.target.value)}
            className="w-full px-2 py-1.5 rounded-lg bg-muted/50 border border-border/50 text-xs outline-none" />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] text-muted-foreground">Monthly Shipments</label>
          <input type="number" value={inputs.monthlyShipments || ''} onChange={e => handleChange('monthlyShipments', e.target.value)}
            className="w-full px-2 py-1.5 rounded-lg bg-muted/50 border border-border/50 text-xs outline-none" />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] text-muted-foreground">Package Type</label>
          <select value={inputs.packagingType || 'corrugated_box'} onChange={e => handleChange('packagingType', e.target.value)}
            className="w-full px-2 py-1.5 rounded-lg bg-muted/50 border border-border/50 text-xs outline-none">
            {Object.entries(PACKAGING_TYPES).map(([k, v]) => <option key={k} value={k}>{v.name}</option>)}
          </select>
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-card/80 to-card/40 backdrop-blur-sm border border-border/50 rounded-xl p-4">
              ...
              <AnimatedGauge value={result.sustainabilityScore} max={100} label="Score" color={result.sustainabilityScore >= 80 ? '#10b981' : result.sustainabilityScore >= 60 ? '#f59e0b' : '#ef4444'} />
            </div>
  ...
  // Unreachable result rendering - kept for reference
  return null
})
