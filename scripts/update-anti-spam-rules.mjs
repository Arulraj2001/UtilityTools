...
const newRules = `=== CONTENT RULES — MANDATORY — VIOLATION = REGENERATE ===
...
// Find the ANTI_SPAM_RULES template literal and replace its content
const startMarker = 'export const ANTI_SPAM_RULES = `\n';
const endMarker = '`\n';

const startIdx = content.indexOf(startMarker);
if (startIdx === -1) {
  console.error('Could not find ANTI_SPAM_RULES start');
  process.exit(1);
}

const afterStart = startIdx + startMarker.length;
// Find the closing backtick after the content
const endIdx = content.indexOf('\n`\n', afterStart);
if (endIdx === -1) {
  console.error('Could not find ANTI_SPAM_RULES end');
  process.exit(1);
}

const newContent = content.slice(0, afterStart) + newRules + content.slice(endIdx + 1);
fs.writeFileSync(path, newContent, 'utf8');
console.log('Done. ANTI_SPAM_RULES updated successfully.');
