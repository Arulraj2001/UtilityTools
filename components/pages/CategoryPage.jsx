      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <section className="mb-8 rounded-xl border border-border/70 bg-card p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-4 text-sm font-medium">
            ...
            <Link href="/blog" className="text-primary hover:underline">Related guides</Link>
            <Link href="/workflow" className="text-primary hover:underline">Workflows</Link>
          </div>
        </section>
          ...
          <section className="mb-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {featuredArticles.map(a => (
                <Link key={a.id} href={`/blog/${encodeURIComponent(a.slug)}`} className="group block rounded-xl border border-border bg-card overflow-hidden hover:shadow-md transition-all premium-card panel-highlight glow-border">
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {jobsForCategory.map(j => (
                <Link key={j.id} href={`/jobs/${encodeURIComponent(j.slug)}`} className="group block rounded-xl border border-border bg-card p-4 hover:shadow-md transition-all">
          ...
          {categoryTools.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {categoryTools.map((tool, i) => (
                <ToolCard key={tool.id} tool={tool} index={i} categoryName={category.name} compact={true} />
              ))}
            </div>
          ) : (
          ...
          <section className="mt-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {relatedWorkflows.slice(0, 6).map(workflow => (
                <Link key={workflow.id} href={`/workflow/${encodeURIComponent(workflow.slug)}`} className="group block rounded-xl border border-border bg-card hover:border-accent/40 hover:shadow-md transition-all premium-card panel-highlight p-4">
          ...
          <section className="mt-10">
            ...
            <div className="flex flex-wrap gap-3">
              {relatedCategories.map(rc => (
                <Link key={rc.id} href={`/category/${encodeURIComponent(rc.slug)}`} className="text-sm px-3 py-1 rounded-full bg-secondary hover:bg-primary/10">{rc.name}</Link>
              ))}
            </div>
          </section>
        )}
