    ...
    <section className="py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-[1.1fr_0.9fr] gap-8 lg:gap-12 items-start">
        <div>
          ...
          <div className="space-y-4 text-muted-foreground leading-relaxed">
            <p>
              ...
              seller calculators, and shipping estimates.
            </p>
            <p>
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          <h2 className="text-xl font-semibold mb-4 text-card-foreground">Who it helps</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {trustLinks.map((link) => (
            <Link key={link.to}
              href={link.to}
              className="block rounded-xl border border-border bg-card p-5 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/20"
            >
              <h3 className="font-semibold mb-2 text-card-foreground hover:text-primary transition-colors">{link.title}</h3>
    ...
    <section className="py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
          ...
          <Link href="/tools" className="text-sm font-medium text-primary hover:text-primary/80">
            View all tools
          </Link>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {popularToolLinks.map((tool) => (
            <Link key={tool.to}
              ...
              className="block rounded-xl border border-border bg-card p-5 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/20"
            >
              <h3 className="font-semibold mb-2 text-card-foreground hover:text-primary transition-colors">{tool.name}</h3>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-[0.85fr_1.15fr] gap-8 lg:gap-12 items-start">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {whyUseQuickUtils.map((item, index) => (
            <div key={index} className="rounded-xl border border-border bg-card p-5 shadow-sm hover:shadow-md transition-all duration-300">
    ...
    <section className="py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="rounded-2xl border border-border bg-card p-6 sm:p-8 shadow-sm">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-card-foreground mb-4">
          ...
          <div className="mt-5">
            <Link href="/privacy" className="text-sm font-semibold text-primary hover:text-primary/80 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {howItWorks.map((step, index) => (
            <div key={step.title} className="rounded-xl border border-border bg-card p-5 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
              <div className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary/10 text-primary mb-4">Step {index + 1}</div>
    ...
    <section className="py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 sm:p-8 text-center shadow-sm">
          ...
          <Link href="/blog"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 shadow-sm transition-colors"
          >
            Explore All Guides
          </Link>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div className="space-y-4">
          {homepageFaq.map((item) => (
            <div key={item.question} className="rounded-2xl border border-border bg-card p-5 shadow-sm hover:shadow-md transition-all duration-300">
              <h3 className="font-semibold mb-2 text-card-foreground">{item.question}</h3>
    ...
    <div>
      ...
      {showCategoriesSectionSkeleton ? (
        ...
        <Suspense fallback={<CategoriesSectionSkeleton />}>
          <HomeCategories categories={priorityCategories} />
        </Suspense>
      )}
        ...
        <section className="py-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {featuredJobs.map((j) => (
                <div key={j.id} className="p-4 rounded-xl border bg-card hover:shadow-md transition">
                  <Link href={`/jobs/${encodeURIComponent(j.slug)}`} className="no-underline">
                    ...
                    <p className="text-sm mt-2 line-clamp-2 text-muted-foreground">{j.short_description}</p>
                  </Link>
                </div>
              ))}
            </div>
