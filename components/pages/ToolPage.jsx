...
import { rankTools, rankBlogPosts } from '@/lib/relevance'
...
export default function ToolPage() {
  ...
  const relatedArticles = useMemo(() => {
    return rankBlogPosts(tool, posts, 6)
  }, [tool, posts])
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <nav className="flex items-center gap-1.5 text-sm text-muted-foreground mb-6 flex-wrap">
          <Link href="/" className="hover:text-foreground flex items-center gap-1"><Home className="w-3.5 h-3.5" /> Home</Link>
          <ChevronRight className="w-3.5 h-3.5" />
          <Link href="/tools" className="hover:text-foreground">Tools</Link>
          {category && (<><ChevronRight className="w-3.5 h-3.5" /><Link href={`/category/${encodeURIComponent(category.slug)}`} className="hover:text-foreground">{category.name}</Link></>)}
          <ChevronRight className="w-3.5 h-3.5" />
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="rounded-2xl border border-border/50 bg-card p-5 sm:p-6">
              ...
              <div className="mt-5 flex flex-wrap gap-4 text-sm font-medium">
                ...
                <Link href="/corrections-policy" className="text-primary hover:underline">Report a correction</Link>
              ...
              <div className="mt-8">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {relatedArticles.map(related => (
                    <Link key={related.id} href={`/blog/${encodeURIComponent(related.slug)}`} className="group block rounded-xl border border-border bg-card hover:border-primary/40 hover:shadow-md transition-all duration-300 overflow-hidden premium-card panel-highlight glow-border">
              ...
              <div className="mt-8">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {workflows.slice(0, 4).map(workflow => (
                    <Link key={workflow.id} href={`/workflow/${encodeURIComponent(workflow.slug)}`} className="group block rounded-xl border border-border bg-card hover:border-accent/40 hover:shadow-md transition-all duration-300 overflow-hidden premium-card panel-highlight">
                      <div className="p-4">
                ...
                <div className="space-y-3">
                  {relatedTools.map((rt, i) => (
                    <Suspense key={rt.id} fallback={null}>
                      <ToolCard tool={rt} index={i} categoryName={category?.name} compact={true} />
                    </Suspense>
                  ))}
                </div>
              </div>
